...
def get_props(saliency):
    """Compute the saliency score proportion of different tokens"""
    ...
    props_sys = saliency[:SYS_LEN].sum()/SYS_LEN
    props_img = saliency[SYS_LEN:SYS_LEN+IMG_LEN].sum()/IMG_LEN
    props_txt = saliency[SYS_LEN+IMG_LEN:].sum()/TXT_LEN
    props4all = [props_sys, props_img, props_txt]

    props_img4img = saliency[SYS_LEN:SYS_LEN+IMG_LEN, SYS_LEN:SYS_LEN+IMG_LEN].sum()
    props_img4txt = saliency[SYS_LEN+IMG_LEN:, SYS_LEN:SYS_LEN+IMG_LEN].sum()
    props4img = [props_img4img, props_img4txt]

    return (props4all, props4img)  